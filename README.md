...
[*quarto-preprint*](https://github.com/mvuorre/quarto-preprint) is a Quarto extension that provides one such output format, called *preprint-typst*. When using this format, Quarto renders your document into PDF using [Typst](https://typst.app/), a modern replacement for LaTeX. *preprint-typst* builds on the standard Quarto Typst template and enables better author-affiliation handling and formatting, allowing for additional (Quarto, Typst, and custom) metadata variables, opinionated typesetting (very), improved two-column layouts (including full-width floats), appendices, and more (read below).
