...
Figures (e.g. @fig-1) are centered by default.
...
You can include page-wide figures (or any other content) in documents that have more than one column. See @fig-2 for an example. First, include
...
:::: {.place arguments='auto, scope: "parent", float: true'}
::: {#fig-2 fig-cap="A full-width figure."}
{{< placeholder 400 200 format=svg >}}
:::
::::
...
To include an appendix (@tip-appendix), include the following [raw Typst](https://quarto.org/docs/output-formats/typst.html#raw-typst) block in your Quarto document, followed by your appendix content:
...
If there are multiple appendices, do not include more than one call to `appendix.with()`. Appendices are automatically numbered with "A.<appendix-number>".

## Citation management {.unlisted .appendix}
